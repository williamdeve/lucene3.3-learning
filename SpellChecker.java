...
public class SpellChecker implements java.io.Closeable {
   ...
   * @throws IOException
   */
  public String[] suggestSimilarWordsAndFilepathByWilliam(String word, int numSug, IndexReader ir,
	      String field, boolean morePopular, float accuracy) throws IOException {
	    // obtainSearcher calls ensureOpen
	    final IndexSearcher indexSearcher = obtainSearcher();
	    try{
	      ...
	      // if the word exists in the real index and we don't care for word frequency, return the word itself
	      if (!morePopular && freq > 0) {
		 //modify by william xu 
	        return new String[] { word+":"+ir.document(ir.termDocs(new Term(field, word)).doc()).get("path") };
	      }
